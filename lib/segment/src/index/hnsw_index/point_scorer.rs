...
impl ConditionChecker for ScorerFilters<'_> {
    ...
    fn check_batched<K: CheckItem>(
        ...
        match select {
            Select::Match => {
                // AND: narrow via the deleted flags, then via the payload filter.
                let n = deleted.check_batched(ids, Select::Match, rest)?;
                match filter_context {
                    Some(f) => f.check_batched(&mut ids[..n], Select::Match, rest),
                    None => Ok(n),
                }
            }
            Select::NonMatch => {
...
impl<'a> FilteredScorer<'a> {
    ...
    pub fn score_points(
        ...
        let mut n = self
            .filters
            .check_batched(point_ids, Select::Match, Rest::Drop)
