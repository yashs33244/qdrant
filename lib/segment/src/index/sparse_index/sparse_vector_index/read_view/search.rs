    ...
    TInvertedIndex: InvertedIndex,
{
    ...
    pub fn search_plain(
        ...
        let point_deleted = vector_query_context
            .deleted_points()
            // TODO(xzfc): check this unwrap_or
            .unwrap_or(self.id_tracker.deleted_point_bitslice());
    ...
    fn search_sparse(
        ...
        let filter_context = filter
            ...
            .transpose()?;
        let mut filter_condition = ScorerFilters::new(filter_context, not_deleted);
        Ok(search_context.search(&mut filter_condition))
    }
