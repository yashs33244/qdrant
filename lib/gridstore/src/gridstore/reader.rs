...
impl<V: Blob, S: UniversalRead> GridstoreReader<V, S> {
    ...
    pub fn max_point_offset(&self) -> PointOffset {
        match &self.variant {
            ...
            ReaderVariant::Serverless(reader) => reader.max_point_offset(),
        }
    }
...
impl<V: Blob, S: UniversalRead> DynamicGridstoreReader<V, S> {
    ...
    fn view(&self) -> DynamicGridstoreView<'_, V, S> {
    ...
    fn max_point_offset(&self) -> PointOffset {
    ...
    fn open<Fs: UniversalReadFs<File = S>>(
        ...
        let tracker = Tracker::<S>::open(fs, &base_path, populate, false)?;
