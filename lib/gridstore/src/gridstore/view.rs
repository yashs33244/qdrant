...
enum ViewVariant<'a, V, S: UniversalRead> {
    Dynamic(DynamicGridstoreView<'a, V, S>),
    Serverless(ServerlessGridstoreView<'a, V, S>),
}

impl<'a, V, S: UniversalRead> GridstoreView<'a, V, S> {
    pub(super) fn from_dynamic(view: DynamicGridstoreView<'a, V, S>) -> Self {
        Self {
    ...
    pub fn max_point_offset(&self) -> PointOffset {
        match &self.variant {
            ...
            ViewVariant::Serverless(view) => view.max_point_offset(),
        }
    }
...
/// [`Tracker<S>`]).
pub(super) struct DynamicGridstoreView<'a, V, S: UniversalRead> {
...
impl<'a, V, S: UniversalRead> DynamicGridstoreView<'a, V, S> {
    pub(crate) fn new(
...
impl<'a, V: Blob, S: UniversalRead> DynamicGridstoreView<'a, V, S> {
